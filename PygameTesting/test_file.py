...
def draw_objects(restart):
    ...
    if (900 <= mouse[0] <= 900 + buttonwidth and 600 <= mouse[1] <= 600 + buttonheight and pressed[0] == True):
        pygame.draw.rect(screen, WHITE, [900, 600, buttonwidth, buttonheight])
        restart = False
    else:
        pygame.draw.rect(screen, BLACK, [900, 600, buttonwidth, buttonheight])

    pygame.display.update()
    return restart

restart = True
#Allows for game to restart
while restart == True:

    # Main game setup
    bg = pygame.image.load("PygameTesting/Images/bg.jpg")
    char1 = pygame.image.load("PygameTesting/Images/char1.png")
    size = (1080, 720)
    screen = pygame.display.set_mode(size)
    pygame.display.set_caption("My First Game")
    clock = pygame.time.Clock()
    font = pygame.font.SysFont("Arial", 30)
    effect1 = pygame.mixer.Sound("PygameTesting/Sounds/randomsound.wav")
    effect2 = pygame.mixer.Sound("PygameTesting/Sounds/soundeffect.wav")
    #pygame.mixer.music.load("PygameTesting/Sounds/musictest.mp3")
